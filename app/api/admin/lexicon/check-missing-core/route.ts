...
/** Unique normalized lemmas, first-seen order (for stable output). */
...
async function fetchExistingLemmaNorms(supabase: ReturnType<typeof createSupabaseAdmin>, norms: string[]): Promise<Set<string>> {
...
/**
 * POST /api/admin/lexicon/check-missing-core
 * Body: { "words": string[] } — compared to lexicon_entries.lemma_norm (exact, after trim+lowercase).
...
export async function POST(req: Request) {
  try {
    ...
    for (const norm of ordered) {
      if (inDb.has(norm)) existing.push(norm);
    ...
    console.error("[admin/lexicon/check-missing-core]", e);
