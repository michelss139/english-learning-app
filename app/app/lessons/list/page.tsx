  ...
  return (
    <main className="space-y-6">
      <header className="rounded-3xl border-2 border-emerald-100/10 bg-emerald-950/40 p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-1">
            <h1 className="text-3xl font-semibold tracking-tight text-white">Lista lekcji</h1>
            <p className="text-base text-emerald-100/80">Twórz i przeglądaj lekcje.</p>
          </div>
      ...
      <section className="rounded-3xl border-2 border-emerald-100/10 bg-emerald-950/40 p-5 space-y-4">
        <div>
          <h2 className="text-lg font-semibold tracking-tight text-white">Dodaj lekcję</h2>
          <p className="text-sm text-white/75">Utwórz lekcję i przypisz zadania.</p>
        </div>

        <div className="rounded-2xl border-2 border-white/10 bg-white/5 p-4 space-y-3">
          {profile?.role === "admin" ? (
            <div className="space-y-1">
              <label className="text-sm font-medium text-white/85">Student ID</label>
              <input
          ...
          <div className="space-y-1">
            <label className="text-sm font-medium text-white/85">Podsumowanie (opcjonalnie)</label>
            <textarea
              className="w-full rounded-2xl border-2 border-white/10 bg-black/10 px-3 py-2 text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-sky-400/30"
              value={lessonSummary}
            ...
            {lessons.map((l) => (
              ...
              >
                <div className="min-w-0">
                  <div className="font-medium text-white truncate">{l.topic}</div>
                  <div className="text-xs text-white/60">{l.lesson_date}</div>
                </div>
