-- Migration: finalize make-do and bring-take cluster normalization after seeds
-- Date: 2026-03-14
-- Goal: enforce the current cluster content standard once seed content is present
...
declare
  ...
  bring_choice_count integer;
  bring_translation_count integer;
...
begin
  ...
  select count(*) into bring_choice_count
  from public.vocab_cluster_questions q
  join public.vocab_clusters c on c.id = q.cluster_id
  where c.slug = 'bring-take' and q.task_type = 'choice';

  select count(*) into bring_translation_count
  from public.vocab_cluster_questions q
  join public.vocab_clusters c on c.id = q.cluster_id
  where c.slug = 'bring-take' and q.task_type = 'translation';
  ...
  if bring_choice_count <> 25 then
    raise exception 'bring-take choice count expected 25, got %', bring_choice_count;
  end if;

  if bring_translation_count <> 15 then
    raise exception 'bring-take translation count expected 15, got %', bring_translation_count;
  end if;
