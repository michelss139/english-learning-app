-- Migration: Fix v_lemma_diagnostics - use SECURITY INVOKER instead of DEFINER
-- Date: 2026-03-06
-- Reason: Supabase Database Linter warns that SECURITY DEFINER may bypass RLS.
--         Explicitly set security_invoker = true so view runs with caller's privileges.

drop view if exists public.v_lemma_diagnostics;

create view public.v_lemma_diagnostics
